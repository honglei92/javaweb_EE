...
<%@ page import="com.doc.entity.User" %>
...
<body class="body">
  ...
  if(userList==null){
	  %>
	  <jsp:forward page="SearchUserServlet"></jsp:forward>
	  <% 
  }
  %>
  <form method="post" action="SearchUserServlet" name="search">
  <table border="1" align="center" style="border-color:green" width="1035"> 
  <tr><td colspan="6">用户一览</td></tr>
  <tr><td colspan="6">用户名：<input type="text" name="username">&nbsp;
  <button onClick="select()">查询</button>&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="addUser.jsp">增加用户</a>
  </td></tr>
  </table>
  </form>
  <form action="DeleteUserServlet" name="del" method="post">
  <input type="hidden" value="" id="username" name="username">
  <table border="1" align="center" style="border-color:green" width="1035">
  <tr align="left"><th>序号</th><th>ID</th><th>用户名</th><th>权限</th><th>登录时间</th><th>操作</th>
  ...
  <%
   for(int i=0;i<userList.size();i++){
	   ...
	   <td align="center"><%=user.getLogtime()%></td>
	   <td align="center"><a href="DeleteUserServlet?id=<%=user.getId() %>">删除用户</a>
	   &nbsp;
	   <a href="updateUser.jsp?id=<%=user.getId() %>">修改权限</a></td>
	   <%
   }
  %>
  </tr>
